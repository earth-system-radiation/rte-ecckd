...
program ecckd_rfmip_sw
  ...
  !
  call load_and_init(ecckd, trim(kdist_file), gas_conc_array(1))
  if(.not. ecckd%source_is_external()) &
    stop "rrtmgp_rfmip_sw: k-distribution file isn't SW"
  nbnd = ecckd%get_nband()
  ngpt = ecckd%get_ngpt()

  allocate(toa_flux(block_size, ecckd%get_ngpt()), &
  ...
  if(top_at_1) then
    ...
    p_lev(:,nlay+1,:) &
                 = ecckd%get_press_min() + epsilon(ecckd%get_press_min())
  end if
